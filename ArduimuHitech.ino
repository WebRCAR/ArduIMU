...
#define MAGNETIC_DECLINATION 2.63    // corrects magnetic bearing to true north
...
void startup_ground(void)
{
  ...
  debug_print("***Waiting GPS to calculate altitude");
  ...
  while (GPS.num_sats <= gps_fix_count && gpsReadsN > 0 && USE_BAROMETER) {
    GPS.update();
...
#if USE_BAROMETER==1
  alt_gnd = GPS.altitude; //Centimeters
  eeprom_busy_wait();
  ...
  pressure_sea = bmp.seaLevelForAltitude((alt_gnd / 100), press_gnd, temp_gnd);
#endif
